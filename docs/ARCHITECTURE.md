...
3. The bridge collects the requests and dispatches each to the matching **asset handler**, which owns dedupe + per-mode write gating and performs the actual item write — but only when neither the bridge master-shadow nor that controller's own `shadowMode()` is engaged (it logs a "would-have-done" line otherwise).
4. Liveness + diagnostic channels are updated.

A change to any car's Mode/Cable/Pause also schedules a debounced (~1 s) tick, so an ECO/SNEL flip re-evaluates promptly instead of waiting up to a full interval. Independently, an optional **shadow energy-management engine** (`ShadowEmsRunner`, see [EMS_ENGINE.md](EMS_ENGINE.md)) runs on the same tick; off by default it only logs, but when `emsOwnsDispatch` is enabled its `SetpointRequest`s replace the `ev-coordinator` + `battery-tou-dispatcher` slots in the dispatch set (the live site runs it this way for EV + battery).
...
- **TariffProvider** — `flat`, `day-night`, `tou-schedule`, `dynamic-spot` (ENTSO-E / Tibber / CSV). Pure functions producing a 24/48 h price schedule.
- **SolarForecastProvider** — Open-Meteo (keyless) or the Forecast.Solar free tier, selected by `kind`; file-cached; rate-limit-aware.
- **EmissionsTracker** — fixed grid factors (`FixedEmissionsProvider`) or live Electricity Maps (`ElectricityMapsProvider`).
...
These are read-only observer controllers under `internal/controller/analytics/` (class names carry a `Controller` suffix, e.g. `CostAnalyticsController`):
