...
| `Evse` | EV charge point (read side): `id()`, `modeName()`, `cableConnected()`, `status()`, `ampsL1..L3()`, `powerW()`, `currentLimitA()`, `paused()` | — |

`EnergyReading`, `Battery` and `ControllableLoad` each ship a small immutable
`record Of(...)` implementation, so readings can be assembled from plain values
and are trivial to fabricate in tests. (`Site`'s shipped implementation is
`ItemSiteReader`'s private snapshot; `Evse` is implemented by `CarSnapshot`.)
...
- **Done:** the capability contracts; the item-backed `ItemSiteReader`; the
  ...
  SoC / reserve / boiler / aircon) flows through `Site`; the `Evse` capability
  (`CarSnapshot` implements it) and `EnergyContext.evses()`, which exposes the
  chargers as typed `Evse` views.
- **Follow-up (not yet done):**
  ...
  - have controllers consume `EnergyContext.evses()` instead of the flat
    per-car fields — the `Evse` capability and accessor now exist, but no
    controller reads them yet;
