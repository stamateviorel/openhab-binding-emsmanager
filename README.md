...
- **Pluggable providers** — tariff (flat / day-night / time-of-use / dynamic spot: ENTSO-E, Tibber, CSV), solar forecast (Open-Meteo or Forecast.Solar), emissions (fixed factors / Electricity Maps).
...
| `forecast-solar` | Thing | Solar production forecast (Open-Meteo keyless, or Forecast.Solar free tier). |
...
Most parameters are documented in `thing-types.xml`; a few advanced ones (CO₂ factors, forecast lat/long, main breaker, battery-sizing) live in `EmsBridgeConfig` and are set via textual `.things` config. The important groups:
...
**Safety / electrical:** `mainBreakerAmpsPerPhase` (per phase; set via `.things`), `gridSafetyMarginW`, `gridEwmaTauSec`, `tickIntervalSeconds`.
...
| 65 | BoilerSchedule | Forces the boiler ON inside user-defined `boilerForceOnSchedule` day:HH:MM windows. |
...
`kind` selects the source: `open-meteo` (keyless; plane-of-array irradiance → PV via `kwp` × `performanceRatio`, capped at `inverterAcKw`) or `forecast-solar-free` (the Forecast.Solar public tier). Common params: `lat`, `lon` (required), `declination` (roof tilt 0–90), `azimuth` (0=south, 90=west, −90=east), `kwp`, `refreshIntervalMin` (default 30 min, min 5; the free tier's 12-calls/hour cap only needs ≥5). Snapshot is file-cached so restarts don't burn API calls. Channels: `nowW`, `next1hWh`/`next3hWh`/`next6hWh`, `todayKwh`, `tomorrowKwh`, `peakTodayAt`, `hourlyForecastTodayCsv`, `forecastSeries` (future points for charts), `lastRefreshAt`, `rateLimitRemaining`, `lastError`.

## `device-meter` Thing

One per sub-metered load. Config: `name` (required), `powerItem` (required), `powerInKw`, `energyItem` (optional/informational), `category`, `color`. Feeds the per-device kWh breakdown and the anomaly detector. The bridge aggregates tracked vs untracked load on `deviceMeterTrackedW`/`deviceMeterUntrackedW`/`deviceMeterTrackedKwhDay`.

## `heatpump` Thing

Config: key items are `powerItem` (for the kWh estimate), `currentTempItem`, `targetTempItem`, `scopCop`, plus reversible-mode params `mode`/`heatModeValue`/`coolModeValue` (see `thing-types.xml` for the full 15-param set). The optimizer learns an RC thermal model online and runs a DP planner to pre-heat in the cheapest hours. Outputs a recommended mode, the next pre-heat window, model R/C/RMSE and a 24 h plan cost.

## `charger` Thing

One per EV. Config: `carKey`, and the per-charger item names (`modeItem`, `cableItem`, `statusItem`, `currentLimitItem`, `pauseItem`, `chargingItem`, `powerOcppItem` / `powerKwItem`, `ampsL1/2/3Item`).

## Examples & architecture

[`examples/`](examples/) contains a complete `demo.things` + `demo.items` for a SolarEdge-class inverter + battery + two EV chargers + a heat pump, plus the recommended persistence strategies. The internal design (the tick loop, `EnergyContext`, controller contract, asset handlers, the statistics tier) is in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md), and the device-capability abstraction (the layer that normalizes item names + sign conventions) in [`docs/CAPABILITIES.md`](docs/CAPABILITIES.md). The `energy:` metadata model and the Kai #3478 engine that backs the live EV + battery dispatch are documented in [`docs/EMS_ENGINE.md`](docs/EMS_ENGINE.md) and [`docs/ENERGY_TAXONOMY.md`](docs/ENERGY_TAXONOMY.md).
