...
public final class EvCoordinatorController implements Controller {
    ...
    private final org.openhab.binding.emsmanager.internal.ems.EvElectrical elec;
    ...
    /** Compatibility constructor using the default effective breaker limit. */
    public EvCoordinatorController(boolean shadowMode, HardPeakShavingController hard, SoftPeakShavingController soft,
        ...
        this(shadowMode, hard, soft, gridSafetyMarginW, CapabilityCheck.EFFECTIVE_LIMIT_A);
    }

    public EvCoordinatorController(boolean shadowMode, HardPeakShavingController hard, SoftPeakShavingController soft,
            int gridSafetyMarginW, int effectiveLimitA) {
        this(shadowMode, hard, soft, gridSafetyMarginW, effectiveLimitA,
                org.openhab.binding.emsmanager.internal.ems.EvElectrical.DEFAULT);
    }

    public EvCoordinatorController(boolean shadowMode, HardPeakShavingController hard, SoftPeakShavingController soft,
            int gridSafetyMarginW, int effectiveLimitA, org.openhab.binding.emsmanager.internal.ems.EvElectrical elec) {
    ...
    private void handleCar(EnergyContext ctx, CarSnapshot car,
        ...
        if ("Charging".equals(status)) {
            ...
            if (attempts <= CHARGE_START_MAX_ATTEMPTS) {
                ...
                if ((attempts - CHARGE_START_MAX_ATTEMPTS) % CHARGE_START_SLOW_RETRY_TICKS == 0) {
                    out.add(new SetpointRequest(car.carKey(), SetpointRequest.Kind.CHARGE_START, 1.0, priority(), NAME,
                            "auto-RemoteStart slow retry (status=" + status + ", attempt " + attempts + ")"));
                }
                return;
            }
        }
        ...
        if (mode == CarSnapshot.Mode.SNEL) {
            int desired = Math.min(elec.maxChargeCurrentA(), headroom);
            ...
            out.add(new SetpointRequest(car.carKey(), SetpointRequest.Kind.PAUSE, 0.0, priority(), NAME,
                    resuming ? "SNEL — resuming after pause" : "SNEL — resume"));
            return;
        }
        ...
        if (ecoBudgetPerCarW != null && !Double.isNaN(ecoBudgetPerCarW)) {
            int targetAmps = (int) Math.floor(ecoBudgetPerCarW / elec.phases() / elec.phaseVoltage());
