...
public final class ForecastSolarHandler extends BaseThingHandler {
    ...
    /**
     * Publish the full-horizon hourly forecast as {@link TimeSeries} future states on the
     * {@code forecastSeries} channel — the provider-profile idea from the #3478 discussion: the
     * provider declares its generation forecast as a time series, and anything downstream
     * (persistence charts, planners, better predictors) builds on it.
     */
    private void publishSeries(String seriesCsv) {
        if (seriesCsv.isEmpty()) {
            return;
        }
        ...
        for (String tok : seriesCsv.split(",")) {
            ...
            try {
                long epochSecond = Long.parseLong(tok.substring(0, eq).trim());
                double w = Double.parseDouble(tok.substring(eq + 1).trim());
                series.add(Instant.ofEpochSecond(epochSecond), new QuantityType<>(w, Units.WATT));
                added++;
            } catch (NumberFormatException e) {
                return; // malformed cached content — skip publishing rather than throw
            }
        }
        if (added > 0) {
            sendTimeSeries(new ChannelUID(getThing().getUID(), FC_CHANNEL_FORECAST_SERIES), series);
        }
    }
