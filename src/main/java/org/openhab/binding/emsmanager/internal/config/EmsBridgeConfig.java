...
public class EmsBridgeConfig {
    ...
    // Soft production-shave band (wired to SoftPeakShavingController AND the engine — both
    // sides always shave with identical parameters).
    public int softShaveThresholdW = -5000;
    public int softRecoveryW = -3000;
    public int softEcoCapA = 8;
    public int normalEcoCapA = 32;

    // Site EV electrical parameters — shared by the EV coordinator, plan estimator and engine.
    // Defaults are the EU 3-phase values; set phases=1 (and your voltage) on single-phase sites.
    public int evPhases = 3;
    public int evPhaseVoltage = 230;
    public int evMaxChargeCurrentA = 32;
    ...
    // Testbed only: item name of the simple consumer to run the informational boiler(demo) parity
    // comparison against (the engine's simple-deadline intent vs the legacy metered planner).
    // Empty (default) = skip. Site configuration — never hardcoded in the engine.
    public String emsParityBoilerItem = "";
    // CUTOVER SWITCH. When true (and emsShadowEnabled), the Kai #3478 engine OWNS the dispatch assets:
    // the four dispatch controllers it replaces (ev-coordinator, solar-surplus-dispatcher, boiler-plan,
    // battery-tou-dispatcher) are suppressed and the engine's SetpointRequests are dispatched through
    // the normal AssetHandler path instead, while the SAFETY controllers and everything else keep
    // dispatching untouched. Off by default = the proven pipeline drives the site; the engine shadows.
    // Only flip after a validated multi-day parity window — it hands live control to the engine.
    public boolean emsOwnsDispatch = false;
    // Site energy-level price windows, in hours out of the 24 h day-ahead schedule (storm.house's
    // percentile windows): the N cheapest hours grade "maximum", the next-cheapest band
    // "encouraged", the M most expensive "restricted". Defaults = the reference's fixed 4/8/4.
    public int levelCheapestHours = 4;
    public int levelCheapHours = 8;
    public int levelExpensiveHours = 4;
    // Seasonal auto-profiles for the windows (winter 8/12/4, transition 6/10/8, summer 4/8/8 —
    // approximating storm.house's seasonal presets). When on, overrides the three fixed windows.
    public boolean levelSeasonalAuto = false;
}
